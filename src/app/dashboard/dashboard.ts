...
import { Subject, takeUntil, forkJoin, timeout, catchError, of, finalize } from 'rxjs';
...
export class Dashboard implements OnInit, OnDestroy, AfterViewInit {
      ...
      .subscribe({
        next: (response: any) => {
          if (response?.success && response.data?.length) {
            ...
            this.activeProjects = this.projects.filter((p: Project) => p.stage === 'operating').length;
  ...
  processFinance(data: any) {
    ...
    const expense = data?.expense ?? data?.totalExpenses ?? 0;
    const netProfit = data?.netProfit ?? revenue - expense;
    const profitMargin =
    ...
    this.totalRevenue = Number(revenue) || 0;
    this.totalExpenses = Number(expense) || 0;
    this.profit = Number(netProfit) || 0;
    this.profitMargin = Number.isFinite(profitMargin) ? Math.round(profitMargin) : 0;

    this.revenueChange = Number(data?.trends?.revenue?.growth ?? data?.revenueChange ?? 0) || 0;
    this.expensesChange = Number(data?.trends?.expense?.growth ?? data?.expensesChange ?? 0) || 0;
  }
  ...
  generateAIInsights() {
    this.aiInsights = [];
    console.log('Generating AI Insights for Project:', this.currentProject?.id);

    // 1. Fetch from Onboarding AI Analysis Result (if available)
    if (this.currentProject?.onboardingData?.aiAnalysisResult) {
      let aiData = this.currentProject.onboardingData.aiAnalysisResult;
      console.log('AI Analysis Result found:', aiData);
      ...
      if (aiData.strategy) {
         this.aiInsights.push({
            type: 'info',
            message: `الاستراتيجية المقترحة: ${aiData.strategy}`,
            action: 'view_plan',
            actionLabel: 'عرض الخطة'
         });
      }

      // Add one of the solutions as an insight
      if (aiData.solutions && Array.isArray(aiData.solutions) && aiData.solutions.length > 0) {
        const sol = aiData.solutions[0];
        this.aiInsights.push({
           type: 'warning',
           message: `تحدي متوقع: ${sol.challenge} - نصيحة: ${sol.advice}`
        });
      }

      // Add roadmap hint
      if (aiData.roadmap && aiData.roadmap.length > 0) {
         this.aiInsights.push({
           type: 'success',
           message: `خارطة الطريق الخاصة بك جاهزة وتحتوي على ${aiData.roadmap.length} خطوات أساسية.`,
           action: 'tasks',
           actionLabel: 'الذهاب للمهام'
         });
      }
      return;
    } else {
      console.warn('No AI Analysis Result found in onboardingData');
    }

    // 2. Fallback local generated insights based on current data
