...
export class TasksAndTeam implements OnInit {
   ...
   * يجلب أعضاء الفريق من الباك إند
   */
  loadTeamMembers(): void {
    if (!this.currentProjectId) return;

    this.teamService.getTeamMembers(this.currentProjectId).subscribe({
      next: (response: any) => {
        if (response.success && response.data) {
          this.teamMembers = response.data.map((m: any) => ({
            id: m.userId,
            name: m.user?.name || 'مستخدم',
            email: m.user?.email,
            role: m.jobTitle || this.getRoleLabel(m.role),
            memberRole: m.role,
            tasks: m.assignedTaskCount || 0,
            avatar: this.getInitial(m.user?.name || '?')
          }));
        }
        
        if (this.teamMembers.length === 0) {
          // Fallback if no members found (should at least have owner)
          this.teamMembers = [{ name: 'أنت', tasks: this.allTasks.length, avatar: 'أ' }];
        }
      },
      error: (err) => {
        console.error('Error loading team members:', err);
        // Fallback to local derivation if API fails
        this.deriveTeamFromTasks();
      }
    });
  }
