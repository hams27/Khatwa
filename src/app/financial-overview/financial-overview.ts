    ...
    this.financeService.getSummary(this.currentProjectId).subscribe({
      next: (response: any) => {
        if (response?.data) {
          ...
          // Backend returns revenue/expenses, frontend expected totalRevenue/totalExpenses
          this.totalRevenue  = d.revenue  || d.totalRevenue  || 0;
          this.totalExpenses = d.expenses || d.totalExpenses || 0;
          this.profit        = d.profit        || 0;
          this.profitMargin  = d.profitMargin  || 0;
          this.updateCards({ ...d, totalRevenue: this.totalRevenue, totalExpenses: this.totalExpenses });
          this.revenueExpenseData.datasets[0].data = [this.totalRevenue, this.totalExpenses, Math.max(this.profit, 0)];
        }
        this.isLoading = false;
        this.cdr.detectChanges();
  ...
  loadFinancialRecords() {
    this.financeService.getRecords(this.currentProjectId, this.startDate, this.endDate).subscribe({
      next: (response: any) => {
        ...
        // Ensure loading is off here too, just in case summary finished first or failed
        this.isLoading = false;
        this.cdr.detectChanges();
      },
      error: () => { 
        this.errorMessage = 'حدث خطأ في تحميل سجل المعاملات'; 
        this.isLoading = false;
        this.cdr.detectChanges();
      }
    });
