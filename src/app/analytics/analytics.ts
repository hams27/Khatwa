...
export class Analytics implements OnInit, OnDestroy, AfterViewInit {
  ...
  loadAnalyticsData() {
    // Using simple counter approach for simplicity without importing forkJoin everywhere
    let completed = 0;
    const checkDone = () => {
      completed++;
      if (completed >= 2) {
        // Fix NG0100: Ensure updates happen in the next change detection cycle
        setTimeout(() => {
          this.isLoading = false;
    ...
    this.taskService.getTasks(this.currentProjectId).subscribe({
      next: (response: any) => {
        if (response?.data) {
          ...
          const rate      = total > 0 ? (completedTasks / total * 100) : 0;
  ...
  generateInsights() {
    ...
    const revenue     = parseFloat(this.statsCards[0].value.replace(/[^0-9.-]+/g, '')) || 0;
    const taskRate    = parseFloat(this.statsCards[1].value.replace(/[^0-9.-]+/g, '')) || 0;
    const growthRate  = parseFloat(this.statsCards[3].value.replace(/[^0-9.-]+/g, '')) || 0;
    ...
    if (this.currentProject?.onboardingData?.aiAnalysisResult) {
      ...
      // Add Strategy
      if (aiData && aiData.strategy) {
         this.insights.push({
            title: 'استراتيجية مقترحة',
            description: aiData.strategy,
            confidence: 90,
            type: 'info'
         });
      }
      ...
      if (aiData && aiData.solutions && Array.isArray(aiData.solutions) && aiData.solutions.length > 0) {
        const sol = aiData.solutions[0];
        this.insights.push({
           title: `تحدي: ${sol.challenge}`,
           description: `نصيحة: ${sol.advice}`,
           confidence: 85,
           type: 'warning'
        });
      }
    ...
    // 2. Add Local Heuristics (if data exists)
    if (growthRate >= 15) {
    ...
    } else if (revenue > 0) { // Only show danger if there IS revenue but it's declining
    ...
    // Only stop loading if we found something OR if completely empty
